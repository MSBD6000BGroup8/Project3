...
import argparse  # Add the ADMMutils module to the import path
import os
import sys
...
model_id = args.model_id; sparsity_function_id = 1;
...
for num_layer,id_layer in enumerate(layer_list):# dual variable, Gamma, and sparsity promoting variable, F, are updated manually and are not trained.
    ...
    V = tf.add(variables_dict['W_%s%s'%(layer_type[num_layer],str(id_layer))],tf.multiply(variables_dict['Gamma%s'%str(num_layer)],1./rho))
    ...
    resF = tf.add(resF, ADMMutils.frobenius_norm(tf.subtract(variables_dict['F%s' % str(num_layer)], operation_dict['F_new%s' % str(num_layer)])))
    resWF = tf.add(resWF, ADMMutils.frobenius_norm(tf.subtract(variables_dict['W_%s%s' % (layer_type[num_layer], str(id_layer))], operation_dict['F_new%s' % str(num_layer)])))
    norm_W_total = tf.add(norm_W_total,
                          ADMMutils.frobenius_norm(variables_dict['W_%s%s' % (layer_type[num_layer], str(id_layer))]))
    norm_F_total = tf.add(norm_F_total, ADMMutils.frobenius_norm(operation_dict['F_new%s' % str(num_layer)]))
    Gamma_assign = variables_dict['Gamma%s'%str(num_layer)].assign(tf.add(variables_dict['Gamma%s'%str(num_layer)] ,tf.multiply(rho ,tf.subtract(variables_dict['W_%s%s'%(layer_type[num_layer],str(id_layer))], operation_dict['F_new%s'%str(num_layer)]))))
...
cross_entropy = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred, labels=label_batch))
penalize_term = ADMMutils.frobenius_norm_square(tf.subtract(tf.subtract(variables_dict['F0'], tf.multiply(variables_dict['Gamma0'], 1. / rho)), variables_dict['W_c1']))
for num_layer in range(1,len(layer_list)):
    U_temp = tf.subtract(variables_dict['F%s'%str(num_layer)],tf.multiply(variables_dict['Gamma%s'%str(num_layer)],1./rho))
    penalize_term = tf.add(penalize_term, ADMMutils.frobenius_norm_square(tf.subtract(U_temp, variables_dict['W_%s%s' % (layer_type[num_layer], str(layer_list[num_layer]))])))
penalized_cross_entropy = tf.add(cross_entropy,tf.multiply(rho,penalize_term))
